...
- 用户管理：新增或批量创建用户、按学号/手机号/QQ/角色/状态筛选、封禁或解封账号、删除用户、重置密码、转让管理员，并查看各用户累计完成页数和任务数
- 审核中心：同意或拒绝超额任务
- 系统设置：修改限额和转让管理员
...
进入“用户管理”，点击“批量创建用户”，阅读格式说明后上传表格或文本文件。
...
TXT 文件每行填写一个学号。用户可在“个人设置”中填写自己的手机号，管理员可在用户列表中查看。

新用户默认密码为学号，并要求首次登录修改密码；账号首次登录前显示为“未使用”，登录后为“正常”。封禁中的账号会立即退出且不能再次登录。
